...
In case of deleting from a singly LL,

-> If head node is given then traverse and then change the next pointers.
-> If ehad node is not given & the node to be deleted is given ,
then simply, paste the node->next in the node and then make
the node->next point to null, so basically we have shifted the
next node to the current node in o(1) time...
